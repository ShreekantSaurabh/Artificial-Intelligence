...
def normalized_columns_initializer(weights, std=1.0):
    out = torch.randn(weights.size())     #randomly initialize the weights and size() gives the no. of elements in the weights. Those elements will be converted to torch tensor.
...
class ActorCritic(torch.nn.Module):
    ...
    def __init__(self, num_inputs, action_space):
        ...
        self.conv1 = nn.Conv2d(num_inputs, 32, 3, stride=2, padding=1) # Input images are fed to first convolution with 32 feature detectors of dimension (kernel size) 3*3, stride = 2 and padding of 1
        self.conv2 = nn.Conv2d(32, 32, 3, stride=2, padding=1) # 32 convoluted output of 1st convolution is fed to second convolution with 32 feature detectors of dimension (kernel size) 3*3, stride = 2 and padding of 1
        ...
        #input to RNN 32 * 3 * 3 is the flattened output of 4th convolution layer. Number of output neuron of LSTM = 256
        num_outputs = action_space.n # getting the number of possible actions
        self.critic_linear = nn.Linear(256, 1) # full connection (neural network) of the critic: output = V(S) . 256 is Number of output neuron of LSTM & 1 is the shared critic output dimension
        self.actor_linear = nn.Linear(256, num_outputs) # full connection (neural network) of the actor: output = Q(S,A) . 256 is Number of output neuron of LSTM output is the no. of actions possible
        ...
        self.critic_linear.weight.data = normalized_columns_initializer(self.critic_linear.weight.data, 1.0) # setting the standard deviation of the critic tensor of weights to 1.0
        #The reason we set larger standard deviation for critic than that of actor is to deal with exploration vs. exploitation
    ...
    def forward(self, inputs):
        inputs, (hx, cx) = inputs # getting separately the input images to the tuple (hidden states, cell states of lstm)
        x = F.elu(self.conv1(inputs)) # forward propagating the signal from the input images to the 1st convolutional layer using Exponential Linear Unit (elu) function which is advanced version of ReLU
