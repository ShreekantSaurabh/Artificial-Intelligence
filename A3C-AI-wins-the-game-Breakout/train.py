...
from envs import create_atari_env     #For Breakout environment
from model import ActorCritic         #imported from our model.py file
...
def train(rank, params, shared_model, optimizer):
    #rank is used to shift the seed so that all training agents are asynchronized
    #params is all the parameters of the environment
    #shared_model is the exploration model by the agent
    ...
    env.seed(params.seed + rank) # aligning the each seed of the environment to the each seed of the agent
    model = ActorCritic(env.observation_space.shape[0], env.action_space) # creating the model from the ActorCritic class
    state = env.reset() # state is a numpy array of size 1*42*42, 1 is for black & white, 42*42 is for no. of pixel at length & breadth of the image
    state = torch.from_numpy(state) # converting the numpy array into a torch tensor
    done = True # when the game is done, so that AI doesn't play the game infinitely
    episode_length = 0 # initializing the length of an episode to 0
    while True: # infinite repeat untill break
        ...
        if done: # if it is the first iteration of the while loop or if the game was just done, then:
            cx = Variable(torch.zeros(1, 256)) # the cell states of the LSTM are reinitialized to 256 zeros of 1 dimension
            hx = Variable(torch.zeros(1, 256)) # the hidden states of the LSTM are reinitialized to 256 zeros of 1 dimension
        else: # else:
        ...
        for step in range(params.num_steps): # going through the num_steps exploration steps
            ...
            entropy = -(log_prob * prob).sum(1, keepdim=True) #Entropy H(p) = - sum_x p(x).log(p(x))
            entropies.append(entropy) # storing the computed entropy
            action = prob.multinomial().data # selecting an action by taking a random draw from the probability distribution
            log_prob = log_prob.gather(1, Variable(action)) # getting the log probability associated to this selected action
